export default function RealtimePage() {
  return (
    <>
      ...
      <pre><code>{`POST https://api.aetherdb.cloud/db/realtime/enable
...
{ "message": "realtime enabled for orders" }`}</code></pre>
      ...
      <pre><code>{`GET https://api.aetherdb.cloud/db/realtime/subscribe?table=orders
Authorization: Bearer <token>
...
const source = new EventSource(
  \`https://api.aetherdb.cloud/db/realtime/subscribe?table=orders&token=\${token}\`
)
...
const source = new EventSource(
  'https://api.aetherdb.cloud/db/realtime/subscribe?table=orders',
