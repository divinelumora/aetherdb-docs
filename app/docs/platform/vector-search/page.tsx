export default function VectorSearchPage() {
  return (
    <>
      ...
      <pre><code>{`POST https://api.aetherdb.cloud/db/vector/column
...
// Response: { "message": "vector column added" }`}</code></pre>
      ...
      <pre><code>{`POST https://api.aetherdb.cloud/db/vector/upsert
Authorization: Bearer <token>
Content-Type: application/json
...
{ "table": "products", "column": "embedding", "id": 42, "embedding": [0.021, -0.134, ...] }`}</code></pre>
      ...
      <pre><code>{`POST https://api.aetherdb.cloud/db/vector/search
Authorization: Bearer <token>
Content-Type: application/json
...
// 2. Search AetherDB
const res = await fetch('https://api.aetherdb.cloud/db/vector/search', {
  method: 'POST',
